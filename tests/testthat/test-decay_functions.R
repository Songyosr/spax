...
create_test_distances <- function() {
  # Create different types of distance inputs
  list(
...
# Test individual decay functions
test_that("gaussian decay works correctly", {
  distances <- seq(0, 50, by = 10)
  weights <- calc_gaussian_decay(distances, sigma = 20)
...
test_that("binary decay works correctly", {
  distances <- seq(0, 100, by = 20)
  threshold <- 50
  weights <- calc_binary_decay(distances, sigma = threshold)
...
test_that("exponential decay works correctly", {
  distances <- seq(0, 50, by = 10)
  weights <- calc_exponential_decay(distances, sigma = 0.1)
...
test_that("power decay works correctly", {
  distances <- seq(1, 50, by = 10)  # Start from 1 to avoid infinity
  weights <- calc_power_decay(distances, sigma = 2)
...
test_that("inverse decay works correctly", {
  distances <- seq(0, 50, by = 10)
  weights <- calc_inverse_decay(distances, c = 1)
...
test_that("calc_decay handles invalid inputs appropriately", {
  ...
  # Test invalid sigma
  # expect_error(
  #   calc_decay(1:5, method = "gaussian", sigma = -1)
  # )
