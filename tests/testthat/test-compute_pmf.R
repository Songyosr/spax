#' @testfile Tests for compute_pmf function
...
test_that("compute_pmf handles zero values correctly", {
...
test_that("compute_pmf handles NA values correctly", {
...
test_that("compute_pmf errors on negative values", {
  # Test raster with negative values
  test_values <- c(-1, 10, 20, 30)
  density_rast <- create_test_raster(test_values, 2, 2)
  expect_error(compute_pmf(density_rast), "Input values cannot be negative")

  # Test raster with all negative values
  all_neg_rast <- create_test_raster(rep(-1, 4), 2, 2)
  expect_error(compute_pmf(all_neg_rast), "Input values cannot be negative")
})

test_that("compute_pmf errors on invalid inputs", {
  # Test invalid input class
  expect_error(compute_pmf(c(1, 2, 3)), "Input must be a SpatRaster object")

  # Test all NA values
  all_na_rast <- create_test_raster(rep(NA, 4), 2, 2)
  expect_error(compute_pmf(all_na_rast), "Input raster contains only NA values")

  # Test all zero values
  all_zero_rast <- create_test_raster(rep(0, 4), 2, 2)
  expect_error(compute_pmf(all_zero_rast), "Total sum is zero")
})

test_that("compute_pmf preserves relative proportions", {
...
test_that("compute_pmf handles different raster sizes", {
...
test_that("compute_pmf maintains spatial properties", {
