...
When preparing demand data for spatial accessibility analysis, we have
several approaches available, each with its own assumptions and
...
In a vector-based approach, we typically represent each area with a
polygon that has an attribute for the number of cases. This often
results in choropleth maps, like this:
...
![](spax-102-data-prep_files/figure-html/unnamed-chunk-2-1.png)

But that could sometimes be misleading since in traditional spatial
analysis, we often allocate demand to only the centroid of each area:
...
![](spax-102-data-prep_files/figure-html/unnamed-chunk-3-1.png)

The centroid approach works for vector-centric analysis, but for `spax`,
we need to think in terms of continuous surfaces. Let’s explore
different ways to convert our data

#### 1. Centroid-Based Approach

The simplest approach treats all demand as concentrated at area
centroids. This mirrors traditional vector-based accessibility analyses
but in raster format:
...
![](spax-102-data-prep_files/figure-html/unnamed-chunk-4-1.png)

As you can see, this approach creates a somewhat odd-looking result -
...
A more sophisticated method distributes cases evenly across each
province’s area:
...
![](spax-102-data-prep_files/figure-html/unnamed-chunk-6-1.png)

This gives us a more continuous surface, but we should validate that our
totals still match:
...
#> 7       TH96  1302       1286.8       15.2      1.2
```

The differences between original and rasterized values arise from the
...
Even more realistic distribution is possible using auxiliary data like
population density to inform where cases are likely to occur within each
province. *This section will be expanded in future versions.*

#### 4. Probabilistic Sampling \[developing\]

For uncertainty analysis, `spax` provides tools to generate multiple
potential demand distributions:
...
The choice of demand preparation method can significantly impact your
accessibility analysis results. Consider your specific context:

- Data availability (Do you have population density data?)
- Scale of analysis (Are districts large enough that distribution
  matters?)
- Purpose (Is point-based analysis sufficient for your needs?)
- Computational resources (Can you handle higher resolution
  distributions?)

### Processing Supply Data

Supply data is usually the most straightforward component - we typically
know exactly where facilities are and their capacities. However, these
locations play a crucial role in the more complex task of distance
...
#> 6 c177  2-General     77     103 (715990.7 752559.6)
```
...
![](spax-102-data-prep_files/figure-html/unnamed-chunk-10-1.png)

### Creating Distance/Travel Time Surfaces

While supply locations themselves are straightforward, calculating
...
we typically calculate from supply points:

1.  We usually have fewer supply points than demand points, making
    computation more efficient
2.  When redistributing accessibility scores back to the population, we
    need supply-based distances anyway
...
![](spax-102-data-prep_files/figure-html/unnamed-chunk-12-1.png)

While simple, this approach ignores real-world travel constraints like
roads and barriers. It’s most useful for:
...
A more realistic approach uses travel time isochrones - areas reachable
within specific time thresholds. While `spax` doesn’t generate
isochrones directly, we can use tools like the
[`osrm`](https://github.com/riatelab/osrm) package:
...
#> 'res' is deprecated, use 'n' instead.
...
![](spax-102-data-prep_files/figure-html/unnamed-chunk-13-1.png)
...
![](spax-102-data-prep_files/figure-html/unnamed-chunk-13-2.png)

The isochrones show travel time “rings” around our facility. If you take
on a vector-based approach, you can use functions like
[`sf::st_contains()`](https://r-spatial.github.io/sf/reference/st_contains.html)
...
![](spax-102-data-prep_files/figure-html/unnamed-chunk-15-1.png)

> ⚠️ **Important Note about OSRM**: By default, the
> [`osrm::osrmIsochrone()`](https://rdrr.io/cran/osrm/man/osrmIsochrone.html)\]
> function uses OSRM’s demo server. This is fine for testing, but for
> real analysis you should:
>
> - [Set up your own OSRM
>   server](https://github.com/Project-OSRM/osrm-backend). *This
>   vignette does not cover the server setup process.*
>
...
Before running any accessibility analysis, it’s crucial to validate your
prepared data. Here’s a systematic approach to validation (some of which
already implemented in the package:) :

##### 1. Spatial Alignment

All raster inputs must align perfectly - same extent, resolution, and
projection:
...
![](spax-102-data-prep_files/figure-html/unnamed-chunk-17-1.png)
...
#> Warning: [summary] used a sample
#>       c172            c173            c174            c175      
#>  Min.   :  6.0   Min.   :  3.0   Min.   :  3.5   Min.   :  4.0  
#>  1st Qu.: 67.5   1st Qu.: 67.5   1st Qu.: 82.5   1st Qu.: 67.5  
#>  Median : 97.5   Median :112.5   Median :127.5   Median :112.5  
#>  Mean   :102.9   Mean   :107.6   Mean   :118.9   Mean   :105.6  
#>  3rd Qu.:142.5   3rd Qu.:142.5   3rd Qu.:157.5   3rd Qu.:142.5  
#>  Max.   :172.5   Max.   :172.5   Max.   :172.5   Max.   :172.5  
#>  NAs    :71579   NAs    :71501   NAs    :74191   NAs    :69709  
#>       c176            c177            c178            c179      
#>  Min.   :  8.5   Min.   : 11.0   Min.   : 11.0   Min.   :  7.5  
#>  1st Qu.: 67.5   1st Qu.: 67.5   1st Qu.: 67.5   1st Qu.: 97.5  
#>  Median :112.5   Median :112.5   Median :112.5   Median :127.5  
#>  Mean   :107.5   Mean   :107.7   Mean   :110.5   Mean   :120.5  
#>  3rd Qu.:142.5   3rd Qu.:142.5   3rd Qu.:157.5   3rd Qu.:157.5  
#>  Max.   :172.5   Max.   :172.5   Max.   :172.5   Max.   :172.5  
#>  NAs    :70996   NAs    :72338   NAs    :74213   NAs    :75822  
#>       c180            c181       
#>  Min.   :  5.0   Min.   :  8.50  
#>  1st Qu.: 97.5   1st Qu.: 67.50  
#>  Median :127.5   Median : 97.50  
#>  Mean   :127.3   Mean   : 96.85  
#>  3rd Qu.:157.5   3rd Qu.:127.50  
#>  Max.   :172.5   Max.   :172.50  
#>  NAs    :76037   NAs    :72831
```

##### 3. Missing Value Analysis

Understand where and why you have NA values; note that not all NAs are
bad:
...
![](spax-102-data-prep_files/figure-html/unnamed-chunk-19-1.png)
...
With your data prepared, you’re ready to move on to actual accessibility
analysis. You can explore the following resources for more information:
