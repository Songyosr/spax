...
Let’s assume we both have at least some passing knowledge of the two-step floating catchment area (2sfca) method. Because BAM! here it is, right in your nose:
...
-   $S_j$ : Service capacity at location j — think: number of doctors, school seats, or happy-hour cocktails 🍸.

-   $P_i$ : Population demand at location i — how many people want the service.

-   $W_d(d_{ij})$ : Distance decay weight between i and j — spoiler: closer = better.

-   $C_j$ : Catchment area around j , aka the “zone of influence.”
...
-   $A_k$ : Accessibility score at demand location k — the higher, the better access.

-   $R_j$ : Supply-to-demand ratio from Step 1.

-   $W_a(d_{kj})$ : Accessibility-side decay weight between k and j .

And there you have it: the 2sfca approach in all its glory. If you’re nodding in recognition (or tilting your head in confusion), no worries. Let’s break it down.
...
The `spax` (short for “spatial access”) package offers tools to analyze spatial accessibility to services, with a particular focus on healthcare. The current version leans heavily on raster-based algorithms. This is a big deal if your data or your analysis workflow is raster-focused—from population surfaces to travel-time rasters. (or at least, you can consider migrating to our side). Some key features of spax include:
...
Ready to dive in? Awesome—let's go!
...
plot(uf_access, main = c("Doctor Accessibility", "Nurse Accessibility"))
```

```{r}
u5_access <- spax_e2sfca(
  demand = pop,
  supply = hc12_hos |> st_drop_geometry(),
  distance = distance_raster,
  id_col = "id",
...
plot(u5_access, main = c("Doctor Accessibility", "Nurse Accessibility"))
```
