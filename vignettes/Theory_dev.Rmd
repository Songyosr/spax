...
3.  **Iterative Equilibrium Computation (Inner Loop)**:\
    ...
    **3.2 Iterative Steps** (for $t = 1,2,\ldots$):
    ...
    c.  **Update Choice and Utilizeing Probabilities**: As shown above,
        if we treat $R_{j}$ as the effective supply at facility $j$, we
        can update the choice probabilities as a function of
        accessibility:

        $$
        Huff_{ij}^{(t)} = \frac{A_{ij}^{(t)}}{\sum_{k \in J} A_{ik}^{(t)}} \newline
        P(i \to j)^{(t)} = Huff_{ij}^{(t)} \cdot f(d_{kj}; \sigma)
        $$

    d.  **recompute Expected Utilization**: We can now update the
    ...
    e.  **Convergence Check**: Repeat until: $$
