...
spread_access(supply_demand_ratio, Wr, plot_prefix = 'access_',
              ratio_cols = c("r_doctors", "r_nurses", "r_rounds", "r_active_hd")) |>
  plot()
  
```

Or we can use the `measure_access()` or `compute_2sfca()` function to calculate accessibility scores directly:

```{r}
supply_mat <- hd[,c("s_total_doctors", "s_total_doctors")] |> st_drop_geometry() |> as.matrix()

access_scores <- measure_access(demand = dialysis_density,
...
hd |> st_drop_geometry() |> data.table::data.table() |> class()
access_scores_2 <- compute_2sfca(demand = dialysis_density,
