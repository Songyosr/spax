...
tiny_pop_density <- (pop_density) |> 
  aggregate(fact = 10, fun = "sum", na.rm=TRUE, #method="simple", 
...
```{r}
# run inner loop
#hd$s_chronic_hd_machines

system.time({
  tmp <- run_accessibility_model(distance_raster, dialysis_density, 
                        supply = hd$s_rounds_per_week, 
                        sigma = 30, max_iter = 300, tolerance = 20, debug = F) 
})
```
```{r}
# Analyze convergence patterns
analysis <- analyze_convergence(tmp$differences, threshold = 0.1)

analysis

# Print summary of problematic facilities
print(analysis$diagnostics[analysis$problematic_facilities,])

# Plot with highlighted problems
plot_convergence_diagnosis(tmp$differences, analysis)
```
...
plot_convergence(tmp$differences)
plot_convergence_summary(tmp$differences)
```
