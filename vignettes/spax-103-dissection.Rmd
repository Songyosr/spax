...
Where: - $P$ is the population density surface - $D_j$ is the travel time/distance surface from facility $j$ - $\mathsf{W_d}$ is the demand-side weighting function - $\odot$ represents element-wise (Hadamard) multiplication - $sum()$ is the global sum over the entire surface - $s_j$ is the supply capacity at facility $j$ - $r_j$ is the resulting ratio for facility $j$
...
Where: - $A$ is now a complete accessibility surface rather than point-specific values. - $\mathsf{W_r}$ is the ratio-side weighting function

This raster-based formulation: - Eliminates the need to iterate over demand points i - Takes advantage of efficient matrix operations - Better reflects how `spax` works with continuous surfaces
