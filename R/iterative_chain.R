...
run_iterative_core_fast <- function(supply, weights, demand,
  ...
  while (iter < max_iter && (!window_filled || mean(diff_window) > tolerance)) {
    ...
    huff_probs <- compute_choice(weights, attractiveness = current_attractiveness, snap = TRUE)
...
run_iterative_core <- function(supply, weights, demand,
  ...
  while (iter < max_iter && (!window_filled || mean(diff_window) > tolerance)) {
    ...
    huff_probs <- compute_choice(weights, attractiveness = current[, 3], snap = TRUE)
    util_probs <- huff_probs * weights
    new_util <- weighted_gather(demand, util_probs, snap = TRUE)$weighted_sum
...
spax_ifca <- function(distance_raster,
  ...
  processed <- preprocess_facilities(supply, distance_raster, snap = snap)
  weights <- compute_weights(processed$distances,
                             method = "gaussian",
                             sigma = sigma,
                             snap = snap)
  ...
  if (snap) {
    utilization <- run_iterative_core_fast(
    ...
    return(reintegrate_facilities(utilization, processed$zero_map))
  }

  # Full computation path
  results <- run_iterative_core(
    supply = processed$supply,
    weights = weights,
    demand = demand,
    lambda = lambda,
  ...
  accessibility <- weighted_spread(raw_ratios, weights, snap = snap)
  util_prob_surface <- sum(results$util_probs, na.rm = TRUE)


  # Reintegrate zeros into final values
  utilization <- reintegrate_facilities(final_state[,1], processed$zero_map)
  ratios <- reintegrate_facilities(final_state[,2], processed$zero_map)
  attractiveness <- reintegrate_facilities(final_state[,3], processed$zero_map)
